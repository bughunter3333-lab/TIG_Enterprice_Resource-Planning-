...
import { Search, Package, Users, User, FileText, BarChart3, Warehouse, Plus, Edit, Trash2, Eye, DollarSign, TrendingUp, ShoppingCart, AlertCircle, X, Calendar, Printer, Download, Bell, Save, Mail, Phone, MapPin, CreditCard, Box, Truck, FileSpreadsheet, LogOut } from 'lucide-react';
...
const TotalImageERP = ({ currentUser, onLogout }) => {
  ...
  // State management for all data (populated via API)
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const [j, inv, cust, sup, po] = await Promise.all([
          api.jobs.list(),
          api.inventory.list(),
          api.customers.list(),
          api.suppliers.list(),
  ...
  const saveJob = async () => {
    try {
      if (editingItem) {
        const updated = await api.jobs.update(editingItem.id, jobForm);
        setJobs(prev => prev.map(j => j.id === updated.id ? updated : j));
        if (selectedJob?.id === updated.id) setSelectedJob(updated);
      } else {
        const created = await api.jobs.create(jobForm);
  ...
  const saveInventoryItem = async () => {
    try {
      if (editingItem) {
        const updated = await api.inventory.update(editingItem.sku, inventoryForm);
        setInventory(prev => prev.map(i => i.sku === updated.sku ? updated : i));
      } else {
        const created = await api.inventory.create(inventoryForm);
  ...
  const saveCustomer = async () => {
    try {
      if (editingItem) {
        const updated = await api.customers.update(editingItem.id, customerForm);
        setCustomers(prev => prev.map(c => c.id === updated.id ? updated : c));
      } else {
        const created = await api.customers.create(customerForm);
  ...
  const adjustStock = async (sku, adjustment, reason) => {
    try {
      const updated = await api.inventory.adjust(sku, adjustment, reason);
      setInventory(prev => prev.map(i => i.sku === updated.sku ? updated : i));
    } catch (e) { setApiError(e.message); }
  };

  const addJobComment = async (jobId, comment) => {
    try {
      await api.jobs.addComment(jobId, comment);
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-500 capitalize">{currentUser?.role?.replace('_', ' ')} • {new Date().toLocaleDateString()}</p>
                </div>
                <button onClick={onLogout} title="Sign out" className="p-2 hover:bg-gray-100 rounded-full text-gray-500 hover:text-red-600 transition-colors">
