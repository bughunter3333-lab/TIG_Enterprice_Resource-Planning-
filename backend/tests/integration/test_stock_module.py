...
def test_create_location(client, make_inventory):
    make_inventory(sku="SKU001")
    resp = client.post(
        "/inventory/SKU001/locations", json={"branch": "HQ", "qty_on_hand": 10}
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_create_duplicate_location_fails(client, make_inventory):
    ...
    resp = client.post(
        "/inventory/SKU001/locations", json={"branch": "HQ", "qty_on_hand": 3}
    )
...
def test_list_locations(client, make_inventory):
    ...
    client.post("/inventory/SKU001/locations", json={"branch": "HQ", "qty_on_hand": 5})
    client.post(
        "/inventory/SKU001/locations", json={"branch": "MELB", "qty_on_hand": 3}
    )
    resp = client.get("/inventory/SKU001/locations")
    assert resp.status_code == 200
...
def test_update_location_bin(client, make_inventory):
    ...
    client.post("/inventory/SKU001/locations", json={"branch": "HQ", "qty_on_hand": 5})
    resp = client.patch(
        "/inventory/SKU001/locations/HQ", json={"primary_bin_1": "Shelf-A1"}
    )
    assert resp.status_code == 200
...
def test_create_price_level_with_breakpoints(client, make_inventory):
    make_inventory(sku="SKU001")
    resp = client.post(
        "/inventory/SKU001/pricing/levels",
        json={
...
def test_create_duplicate_price_level_fails(client, make_inventory):
    ...
    client.post(
        "/inventory/SKU001/pricing/levels",
        json={"price_level": "1-Price A", "breakpoints": []},
    )
    resp = client.post(
        "/inventory/SKU001/pricing/levels",
        json={"price_level": "1-Price A", "breakpoints": []},
...
def test_update_cost_tracking(client, make_inventory):
    make_inventory(sku="SKU001")
    resp = client.put(
        "/inventory/SKU001/pricing/cost", json={"last_cost": 30.12, "avg_cost": 30.12}
    )
    assert resp.status_code == 200
...
def test_transactions_with_job_fk(client, make_inventory, make_customer, db):
    ...
    job = Job(
        id="J001", customer_id="CUST001", customer_name="Test Customer", status="ORDER"
    )
    db.add(job)
    db.commit()
    mv = StockMovement(
        sku="SKU001", date="10/06/2026", type="Sale", quantity=-2, job_id="J001"
    )
    db.add(mv)
    db.commit()
    resp = client.get("/inventory/SKU001/transactions")
...
def test_committed_from_job_items(client, make_inventory, make_customer, db):
    ...
    make_customer(id="CUST001")
    job = Job(
        id="J002", customer_id="CUST001", customer_name="Test Customer", status="ORDER"
    )
    db.add(job)
    db.commit()
    item = JobItem(
...
def test_committed_excludes_paid_jobs(client, make_inventory, make_customer, db):
    ...
    make_customer(id="CUST001")
    job = Job(
        id="J003", customer_id="CUST001", customer_name="Test Customer", status="PAID"
    )
    db.add(job)
    db.commit()
