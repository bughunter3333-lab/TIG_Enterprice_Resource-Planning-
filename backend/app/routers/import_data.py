...
CUSTOMER_MAP = {
    ...
    "id": ["id", "customer_id", "customerid", "code", "customer_code", "account_no",
           "accountno", "cust#", "cust_no", "card_code", "card#", "cardcode"],
    # Jim2: "Name"
    "name": ["name", "customer_name", "customername", "company", "company_name",
             "business_name", "businessname", "trading_name"],
    # Jim2: "Contact Person"
    ...
    "address": ["address", "billing_address", "billingaddress", "street_address",
    ...
    # Jim2: "Credit Limit $"
    "credit_limit": ["credit_limit", "creditlimit", "credit", "credit_limit_$",
                     "credit_limit$"],
    # Jim2: "Account Manager" / "Acc. Mgr" / "Acc Mgr"
    "account_manager": ["account_manager", "accountmanager", "rep", "sales_rep",
                        "salesrep", "acc_mgr", "acc._mgr", "acc.mgr",
                        "account_mgr", "acct_mgr"],
    "status": ["status", "active", "account_status"],
}

SUPPLIER_MAP = {
    "id": ["id", "supplier_id", "supplierid", "code", "supplier_code",
           "suppliercode", "vend#", "vendor_code", "vendor#"],
    "name": ["name", "supplier_name", "suppliername", "company", "company_name",
...
INVENTORY_MAP = {
    ...
    "sku": ["sku", "code", "product_code", "productcode", "item_code", "itemcode",
            "part_number", "partno", "stock_code", "stockcode"],
    # Jim2: "ShortDesc" / "Desc" / "Description"
    "name": ["name", "description", "item_name", "itemname", "product_name",
             "productname", "item_description", "shortdesc", "short_desc",
             "item_desc", "desc", "title"],
    # Jim2: "StockGroup1"
    "category": ["category", "product_category", "type", "group",
                 "stockgroup1", "stock_group1", "stockgroup", "stock_group",
                 "prod_group", "item_group"],
    "supplier": ["supplier", "supplier_name", "vendor", "vendor_name",
                 "supplier_code"],
    # Jim2: "On Hand"
    "stock": ["stock", "qty", "quantity", "on_hand", "onhand", "stock_qty",
              "current_stock", "qty_on_hand"],
    # Jim2: "Reorder Level"
    "min_stock": ["min_stock", "minstock", "reorder_level", "reorderlevel",
                  "reorder", "minimum_stock", "min_qty"],
    # Jim2: "SupplierPrice" / "Cost Price"
    "unit_cost": ["unit_cost", "unitcost", "cost", "buy_price", "buyprice",
                  "cost_price", "costprice", "supplierprice", "supplier_price"],
    # Jim2: "RRPInc" / "Sell Price"
    "sell_price": ["sell_price", "sellprice", "unit_price", "unitprice", "price",
                   "selling_price", "rrpinc", "rrp_inc", "rrp", "retail_price"],
    "location": ["location", "bin", "bin_location", "binlocation",
                 "warehouse_location", "slot"],
    "status": ["status"],
}

JOB_MAP = {
    # Jim2: "Job#"
    "id": ["id", "job_id", "jobid", "job_number", "jobnumber", "job_no",
           "jobno", "job#"],
    # Jim2: "Cust#"
    "customer_id": ["customer_id", "customerid", "customer_code", "customercode",
                    "account_no", "cust#", "cust_no", "card_code"],
    "customer_name": ["customer_name", "customername", "customer", "company",
                      "client", "cust_name"],
    "status": ["status", "job_status"],
    # Jim2: "Inv#"
    "invoice": ["invoice", "invoice_number", "invoicenumber", "invoice_no",
                "invoice#", "inv#", "inv_no"],
    # Jim2: "Date In"
    "date_in": ["date_in", "datein", "date", "order_date", "orderdate",
                "start_date", "date_created"],
    # Jim2: "Date Due"
    "due": ["due", "due_date", "duedate", "delivery_date", "required_date",
            "date_due", "datedue"],
    # Jim2: "Date Out"
    "out": ["out", "out_date", "dispatch_date", "dispatched", "shipped_date",
            "date_out"],
    "priority": ["priority"],
    "type": ["type", "job_type", "jobtype"],
    # Jim2: "Name" (responsible staff) / "Acc. Mgr"
    "assigned_to": ["assigned_to", "assignedto", "assignee", "staff",
                    "acc_mgr", "acc._mgr", "acc.mgr", "account_manager",
                    "acct_mgr", "name", "responsible"],
    # Jim2: "Total Ex."
    "total_ex": ["total_ex", "totalex", "subtotal", "subtotal_$", "amount_ex",
                 "net_amount", "total_ex.", "sub_total", "ex_gst", "ex_tax"],
    # Jim2: "Tax Paid" / "TaxContent"
    "tax": ["tax", "gst", "tax_amount", "tax_$", "tax_paid", "taxcontent",
            "tax_content", "gst_amount"],
    # Jim2: "Total Inc."
    "total_inc": ["total_inc", "totalinc", "total", "total_$_(aud)",
                  "total_$(aud)", "amount_inc", "gross_amount",
                  "invoice_amount", "total_inc.", "inc_gst", "inc_tax",
                  "total_including_tax"],
    # Jim2: "Invoice Paid $"
    "deposit": ["deposit", "paid", "invoice_paid", "amount_paid",
                "payment_received", "invoice_paid_$", "invoice_paid$",
                "paid_$"],
    # Jim2: "Balance Due $"
    "balance_due": ["balance_due", "balancedue", "balance", "outstanding",
                    "balance_due_$", "balance_due$", "amount_outstanding"],
    "payment_method": ["payment_method", "paymentmethod", "payment_type"],
    "shipping_address": ["shipping_address", "shippingaddress", "ship_address",
                         "delivery_address"],
    "notes": ["notes", "note", "comments", "remarks", "instructions",
              "job_notes"],
    # Jim2: "Cust Ref#"
    "cust_ref": ["cust_ref", "custref", "cust_ref#", "customer_reference",
                 "customer_ref", "cust_ref_no", "po_number", "cust_po"],
    # Jim2: "OurRef"
    "our_ref": ["our_ref", "ourref", "our_ref#", "our_reference",
                "our_ref_no", "internal_ref"],
    # Jim2: "Item Desc" / "Invoice Description"
    "description": ["description", "desc", "desc.", "job_description", "job_desc",
                    "item_desc", "invoice_description", "invoice_desc"],
    # Jim2: "Ship#"
    "ship_to": ["ship_to", "shipto", "ship#", "ship_no", "ship_code",
                "shipping_code"],
    # Jim2: "Project#" (not directly in Jim2 job list but used in custom exports)
    "project_no": ["project_no", "projectno", "project#", "project_number",
                   "project"],
    "serial_no": ["serial_no", "serialno", "serial#", "serial_number", "serial"],
    # Jim2: "Name/Contact" (customer contact on job)
    "name_contact": ["name_contact", "name/contact", "contact_name",
                     "contactname", "customer_contact"],
}

CARD_FILE_MAP = {
    # Jim2 ship card: "Card Code"
    "ship_code": ["ship_code", "shipcode", "ship#", "card_code", "code",
                  "ship_to_code", "delivery_code"],
    # Billing customer code: "Cust#" or derived
    "customer_code": ["customer_code", "cust#", "cust_code", "account_code",
    ...
    "address1": ["address1", "address_1", "address", "street", "street_address",
                 "postal_address", "delivery_address"],
...
def map_row(row: dict, field_map: dict[str, list[str]]) -> dict:
    ...
    lowered = {normalise_key(k): (v.strip() if isinstance(v, str) else v)
    ...
    for i, row in enumerate(rows, 1):
        ...
        try:
            ...
            if not mapped.get("sku") or not mapped.get("name"):
                errors.append({"row": i, "error": "Missing required fields: sku or name"})
                skipped += 1
                continue

            existing = db.query(InventoryItem).filter(InventoryItem.sku == mapped["sku"]).first()
...
async def import_card_files(
    ...
    for i, row in enumerate(rows, 1):
        ...
        try:
            ...
            existing = db.query(CardFile).filter(CardFile.ship_code == ship_code).first()
...
async def preview_csv(file: UploadFile = File(...), _: User = Depends(require_staff)):
    ...
    data_rows = [r for r in rows if any(v and str(v).strip() for v in r.values())
