...
def gst_report(
    ...
    for j in jobs:
        ...
        if month not in monthly:
            monthly[month] = {"month": month, "invoices": 0, "sales_inc": 0.0, "sales_ex": 0.0, "gst": 0.0}
...
def aged_payables(
    ...
    for po in pos:
        ...
        due_dt = _parse_date(getattr(po, "due_date", None) or po.order_date or "", as_at_date)
    ...
    for r in rows:
        for k in ("current", "days_31_60", "days_61_60", "days_61_90", "over_90", "total"):
            if k in r:
                r[k] = round(r[k], 2)
...
def inventory_valuation(
    ...
    for i in items:
        ...
        if cat not in by_category:
            by_category[cat] = {"category": cat, "sku_count": 0, "total_units": 0, "value": 0.0}
    ...
    return {
        "rows": rows,
        "totals": {"sku_count": grand_skus, "total_units": grand_units, "value": grand_total},
    }
...
def job_profitability(
    ...
    q = db.query(Job).options(joinedload(Job.items)).filter(
        Job.status.in_(["INVOICE", "PAID", "FINISH"])
    )
    if date_from:
    ...
    for j in jobs:
        ...
        cost = sum(float(it.purchase_price or 0) * int(it.order_qty or 0) for it in product_items)
    ...
    totals = {
        ...
        "margin_pct": round(
            sum(r["margin"] for r in rows) / sum(r["revenue_ex"] for r in rows) * 100
            if sum(r["revenue_ex"] for r in rows) > 0 else 0, 1),
    }
...
def on_time_delivery(
    ...
    for j in jobs:
        ...
        cid = j.customer_id or "UNKNOWN"
        if cid not in by_customer:
            by_customer[cid] = {"customer_id": cid, "customer_name": j.customer_name or "Unknown",
                                 "total": 0, "on_time": 0, "late": 0, "no_due_date": 0}
    ...
    return {
        "summary": {"total": total, "on_time": on_time, "late": late,
                    "no_due_date": no_due_date, "rate": rate},
        "by_customer": rows,
        "date_from": date_from,
        "date_to": date_to,
    }
...
def decoration_performance(
    ...
    for j in jobs:
        for item in j.items:
            ...
            if dec not in by_dec:
                by_dec[dec] = {"decoration_type": dec, "item_count": 0, "qty": 0,
            ...
            by_dec[dec]["cost"] += float(item.purchase_price or 0) * int(item.order_qty or 0)
    ...
    for r in rows:
        ...
        r["margin_pct"] = round((r["margin"] / r["revenue"] * 100) if r["revenue"] > 0 else 0, 1)
