...
def get_forecast(db: Session = Depends(get_db), _: User = Depends(require_any)):
    ...
    trend_dir = (
        "upward" if holt_trend > 50 else "downward" if holt_trend < -50 else "flat"
    )
    avg_3 = sum(ys[-3:]) / 3
...
def get_dso(db: Session = Depends(get_db), _: User = Depends(require_any)):
    ...
    all_jobs = db.query(Job).all()
    invoiced_jobs = [
        j for j in all_jobs if j.status in ("INVOICE", "PAID") and j.date_in
    ]
    ...
    for (cid, cname), bal in sorted(cust_bal.items(), key=lambda x: -x[1])[:10]:
        top_receivables.append(
            {"customer_id": cid, "customer_name": cname, "balance_due": round(bal, 2)}
        )
...
def get_churn_risk(db: Session = Depends(get_db), _: User = Depends(require_any)):
    ...
    for j in all_jobs:
        ...
        if d and j.customer_id:
            ...
            total_spend[j.customer_id] = total_spend.get(j.customer_id, 0.0) + float(
                j.total_inc or 0
            )
        ...
        customer_shares.append(
            {
                ...
                "revenue_share_percentage": (
                    round(rev / total_rev_inc * 100, 1) if total_rev_inc else 0
                ),
            }
        )
...
def get_seasonality(db: Session = Depends(get_db), _: User = Depends(require_any)):
    ...
    slow_month_idx = min(
        (m for m in avgs if avgs[m] > 0), key=lambda k: avgs[k], default=1
    )
