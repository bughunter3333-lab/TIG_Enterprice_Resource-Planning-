...
def _build_pdf(job: Job, company: CompanySettings, doc_type: str) -> bytes:
    ...
    h2 = S("h2", fontSize=11, textColor=DARK, fontName="Helvetica-Bold")
    ...
    right_bold = S("rb", fontSize=9, textColor=DARK, alignment=TA_RIGHT, fontName="Helvetica-Bold")
    ...
    header_data = [
        [
            ...
            Paragraph(title, ParagraphStyle("title", fontSize=20, textColor=ACCENT,
    ...
    info_data = [
        [
            "\n".join([f"<para>{l}</para>" for l in co_lines]),
    ...
    # ── Bill To ───────────────────────────────────────────────────────────────
    bill_to_data = [
        [Paragraph("BILL TO", S("bt", fontSize=8, textColor=ACCENT, fontName="Helvetica-Bold"))],
        [Paragraph(f"<b>{job.customer_name or ''}</b>", normal)],
    ]
    if job.cust_ref:
        bill_to_data.append([Paragraph(f"Ref: {job.cust_ref}", small)])
    if job.shipping_address:
        bill_to_data.append([Paragraph(job.shipping_address.replace("\n", "<br/>"), small)])
    ...
    item_header = ["Description", "Qty", "Unit Price", "Discount", "Total Ex", "Total Inc"]
    ...
    product_items = [it for it in (job.items or []) if it.display_type not in ("section", "note")]

    for it in product_items:
        qty = int(it.order_qty or 0)
        price_ex = float(it.price_ex or 0)
        price_inc = float(it.price_inc or price_ex * 1.1)
    ...
    for i, (_, label, amount) in enumerate(totals_rows):
        ...
        ts_rows.append(["", Paragraph(label, label_style), Paragraph(amount, amount_style)])
    ...
    grand_idx = next((i for i, r in enumerate(ts_rows) if "BALANCE DUE" in str(r[1]) or "Total (inc GST)" in str(r[1])), None)
    ...
    story.append(Paragraph(footer_text, S("ft", fontSize=7, textColor=colors.HexColor("#94a3b8"),
                                           alignment=TA_CENTER)))
...
def _build_picking_slip(job: Job, company: CompanySettings) -> bytes:
    ...
    hdr = Table([[
        Paragraph(company.company_name or "Total Image Group", h1),
        Paragraph("PICKING SLIP", ParagraphStyle("ps_title", fontSize=20,
    ...
    # Job meta grid
    meta = Table([
        [Paragraph("JOB #", label), Paragraph(job.id, bold),
         Paragraph("CUSTOMER", label), Paragraph(job.customer_name or "", bold)],
        [Paragraph("DUE", label), Paragraph(job.due or "—", normal),
         Paragraph("CUST REF", label), Paragraph(job.cust_ref or "—", normal)],
        [Paragraph("BRANCH", label), Paragraph(getattr(job, "branch", "") or "HQ", normal),
         Paragraph("ASSIGNED", label), Paragraph(job.assigned_to or "—", normal)],
    ...
    if job.shipping_address or job.ship_to:
        ...
        story.append(Paragraph("<b>SHIP TO:</b>", S("stl", fontSize=8, textColor=ACCENT, fontName="Helvetica-Bold")))
        if ship_label:
            story.append(Paragraph(ship_label, bold))
        if ship_addr:
            story.append(Paragraph(ship_addr, S("sa", fontSize=8, textColor=DARK, leading=11)))
        story.append(HRFlowable(width="100%", thickness=0.5, color=BORDER, spaceBefore=3, spaceAfter=4))

    # Items table
    product_items = [it for it in (job.items or []) if it.display_type not in ("section", "note")]
    col_widths = [22*mm, 65*mm, 22*mm, 22*mm, 22*mm, 22*mm]
    rows = [[Paragraph(h, th) for h in ["SKU", "Description", "Location", "Ordered", "Picked", "Status"]]]
    for it in product_items:
        rows.append([
    ...
    zero_weight = sum(1 for it in product_items if it.stock_code and not float(getattr(it, "weight_kg", 0) or 0))
    footer_rows = []
    ...
    if job.notes:
        footer_rows.append([Paragraph("<b>Notes:</b>", normal),
                            Paragraph(job.notes.replace("\n", "<br/>"), S("fnotes", fontSize=8, textColor=DARK))])
    if footer_rows:
...
def _build_job_sheet(job: Job, company: CompanySettings) -> bytes:
    ...
    hdr = Table([[
        Paragraph(company.company_name or "Total Image Group", h1),
        Paragraph("JOB SHEET", ParagraphStyle("js_title", fontSize=20,
                  textColor=colors.HexColor("#dc2626"), fontName="Helvetica-Bold", alignment=TA_RIGHT)),
    ]], colWidths=["60%", "40%"])
    hdr.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "MIDDLE")]))
    story.append(hdr)
    story.append(HRFlowable(width="100%", thickness=2, color=colors.HexColor("#dc2626"), spaceAfter=4))

    meta = Table([
        [Paragraph("JOB #", label), Paragraph(job.id, bold),
         Paragraph("CUSTOMER", label), Paragraph(job.customer_name or "", bold)],
        [Paragraph("DUE", label), Paragraph(job.due or "—", normal),
         Paragraph("TYPE", label), Paragraph(job.type or "Normal", normal)],
        [Paragraph("BRANCH", label), Paragraph(getattr(job, "branch", "") or "HQ", normal),
         Paragraph("ASSIGNED TO", label), Paragraph(job.assigned_to or "—", normal)],
        [Paragraph("DESCRIPTION", label), Paragraph(job.description or "—", normal),
         Paragraph("CUST REF", label), Paragraph(job.cust_ref or "—", normal)],
    ...
    rows = [[Paragraph(h, th) for h in ["Description / SKU", "Qty", "Decoration", "Code", "Notes / Placement"]]]
    for it in (job.items or []):
        if it.display_type == "section":
            # Section header spanning all columns
            rows.append([Paragraph(f"<b>{it.description or ''}</b>",
                                   ParagraphStyle("sec_js", fontSize=9, textColor=ACCENT,
    ...
    tbl.setStyle(TableStyle([
        ...
        ("RIGHTPADDING", (0, 0), (-1, -1), 5),
        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
        ("SPAN", (0, 1), (-1, 1)) if any(it.display_type == "section" for it in (job.items or [])) else ("", (0,0), (0,0)),
    ]))
    story.append(tbl)
    story.append(HRFlowable(width="100%", thickness=0.5, color=BORDER, spaceBefore=4, spaceAfter=4))
    ...
    if job.proof_notes:
        ...
        story.append(Paragraph(job.proof_notes.replace("\n", "<br/>"),
...
def _build_statement(customer: Customer, lines: list, totals: dict, company: CompanySettings,
    ...
    bold = S("b", fontSize=9, textColor=DARK, fontName="Helvetica-Bold", leading=13)
    right = S("r", fontSize=9, textColor=DARK, alignment=TA_RIGHT)
    right_bold = S("rb", fontSize=9, textColor=DARK, fontName="Helvetica-Bold", alignment=TA_RIGHT)
    th = ParagraphStyle("th_st", fontSize=8, textColor=colors.white, fontName="Helvetica-Bold")

    story = []

    hdr = Table([[
        Paragraph(company.company_name or "Total Image Group", h1),
        Paragraph("STATEMENT OF ACCOUNT", ParagraphStyle("soa_title", fontSize=16,
                  textColor=ACCENT, fontName="Helvetica-Bold", alignment=TA_RIGHT)),
    ]], colWidths=["55%", "45%"])
    hdr.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "MIDDLE")]))
    story.append(hdr)
    ...
    meta_lines = [
        Paragraph(f"Statement Date: <b>{date.today().strftime('%d/%m/%Y')}</b>", normal),
    ]
    if date_from or date_to:
        period = f"{date_from or ''} – {date_to or 'present'}"
        meta_lines.append(Paragraph(f"Period: <b>{period}</b>", normal))
    if customer.payment_terms:
        meta_lines.append(Paragraph(f"Payment Terms: <b>{customer.payment_terms}</b>", normal))
    if company.abn:
        meta_lines.append(Paragraph(f"ABN: {company.abn}", small))

    info_tbl = Table([[bill_lines, meta_lines]], colWidths=["55%", "45%"])
    info_tbl.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "TOP"), ("RIGHTPADDING", (0, 0), (0, -1), 10)]))
    story.append(info_tbl)
    story.append(HRFlowable(width="100%", thickness=0.5, color=BORDER, spaceBefore=6, spaceAfter=5))

    # Transactions table
    col_widths = [22*mm, 22*mm, 22*mm, 65*mm, 22*mm, 22*mm, 22*mm]
    rows = [[Paragraph(h, th) for h in ["Date", "Due Date", "Invoice #", "Description", "Amount", "Paid", "Balance"]]]
    running = 0.0
    for line in lines:
        ...
        rows.append([
            ...
            Paragraph(line.get("invoice_no") or line.get("job_id") or "—", S("td3", fontSize=8, textColor=DARK, fontName="Helvetica-Bold")),
            Paragraph(line.get("description") or "", S("td4", fontSize=8, textColor=DARK)),
    ...
    summary_rows = [
        ["", Paragraph("Total Invoiced:", right), Paragraph(f"${float(totals.get('invoiced', 0)):,.2f}", right)],
        ["", Paragraph("Total Paid:", right), Paragraph(f"${float(totals.get('paid', 0)):,.2f}", right)],
        ["", Paragraph("<b>AMOUNT DUE:</b>", right_bold),
         Paragraph(f"<b>${outstanding:,.2f}</b>",
                   S("due", fontSize=10, textColor=colors.HexColor("#dc2626") if outstanding > 0 else DARK,
                     fontName="Helvetica-Bold", alignment=TA_RIGHT))],
    ]
    ...
    story.append(HRFlowable(width="100%", thickness=0.5, color=BORDER, spaceBefore=6, spaceAfter=4))
    ...
    if company.abn:
        footer_text += f"  |  ABN {company.abn}"
    story.append(Paragraph(footer_text, S("ft", fontSize=7, textColor=colors.HexColor("#94a3b8"), alignment=TA_CENTER)))
...
def customer_statement_pdf(
    ...
    company = db.query(CompanySettings).first() or CompanySettings(company_name="Total Image Group")
